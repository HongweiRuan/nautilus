...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one nuPlan discontinuity eval shard.")
    ...
    parser.add_argument("--diffusion-planner-root", default=os.environ.get("DIFFUSION_PLANNER_ROOT", "/hugsim-storage/Diffusion-Planner"))
    parser.add_argument(
        "--args-file",
        default=os.environ.get("DIFFUSION_PLANNER_ARGS", "/closed-loop-e2e/weights/nuplan/diffusionplanner/args.json"),
    )
    parser.add_argument(
        "--ckpt-path",
        default=os.environ.get("DIFFUSION_PLANNER_CKPT", "/closed-loop-e2e/weights/nuplan/diffusionplanner/model.pth"),
    )
    parser.add_argument("--exp-root", default=os.environ.get("NUPLAN_EXP_ROOT", "/hugsim-storage/diffusion_planner_exp"))
    parser.add_argument("--manifest-dir", default=os.environ.get("SHARD_MANIFEST_DIR", "/hugsim-storage/diffusion_planner_exp/shards"))
...
def query_tokens_in_db(db_file: Path, wanted_tokens: Set[str]) -> List[str]:
    ...
    tokens = []
    for (token_hex,) in rows:
        token = token_hex.lower()
...
def build_or_load_shards(args: argparse.Namespace) -> List[List[DbShardItem]]:
    ...
    filter_path = Path(args.diffusion_planner_root) / "diffusion_planner" / "config" / "scenario_filter" / f"{args.scenario_filter}.yaml"
...
def run() -> int:
    ...
    experiment_uid = (
        f"diffusion_planner_discontinuity/{args.scenario_filter}/{args.simulation}/"
    ...
    cmd = [
        ...
        "planner=replan_diffusion_planner",
        f"planner.replan_diffusion_planner.replan_interval_steps={replan_step}",
        f"planner.replan_diffusion_planner.base_planner.config.args_file={args.args_file}",
        f"planner.replan_diffusion_planner.base_planner.ckpt_path={args.ckpt_path}",
    ...
    return subprocess.run(cmd, env=env, cwd=args.diffusion_planner_root).returncode
