...
import json,sys
rows=json.load(open(sys.argv[1])); requested=[t for t in __import__('os').environ.get('RUN_TOKENS','').split(',') if t]; tokens=requested or sorted({r['token'] for r in rows}); token=tokens[int(sys.argv[2])]; events={e for e in __import__('os').environ.get('RUN_EVENTS','').split(',') if e}
for r in rows:
 if r['token']==token and (not events or r['event'] in events): print('\t'.join(str(r.get(k) or '-') for k in ['leaf','token','event','recipe','data_root','asset_harvester_manifest','replay_frames']))
PYROWS
)
[ "${#CELLS[@]}" -eq "${EXPECTED_CELLS:-2}" ] || { say "unexpected cell count: ${#CELLS[@]}"; exit 1; }
...
HARMONIZER_ARGS=()
if [ "${ENABLE_HARMONIZER:-1}" != 0 ]; then HARMONIZER_ARGS=(--enable-harmonizer --harmonizer-cache /avl-west/navsafe_dev/.harmonizer-cache); fi
say "renderer harmonizer=${ENABLE_HARMONIZER:-1} asset_replace_max=${NUREC_GRPC_ASSET_REPLACE_MAX:-10}"
CUDA_VISIBLE_DEVICES=0 /app/run serve-grpc --host 0.0.0.0 --enable-editing-actors --renderer default --cache-size 4 "${HARMONIZER_ARGS[@]}" --artifact-glob '/root/shard/*.usdz' > /tmp/renderer.log 2>&1 &
SERVE_PID=$!
