...
import json,sys
rows=json.load(open(sys.argv[1])); tokens=sorted({r['token'] for r in rows}); token=tokens[int(sys.argv[2])]
for r in rows:
...
export NEXUSSIM_NO_CAM_MAP_LINES=1 NEXUSSIM_NO_OVERLAY=0 UV_NO_SYNC=1 LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6 NEXUSSIM_NUREC_GPUS=1
...
for ROW in "${CELLS[@]}"; do
  ...
  if CUDA_VISIBLE_DEVICES=$EVAL_GPU "$PY" "$REPO/scripts/tools/eval_py123d.py" \
      ...
      --traffic-mode navsafe --recipe "$REPO/nexussim/navsafe/recipes/proxy_set_state_perturbation/$RECIPE" \
  ...
  if [ "$ok" = 1 ]; then say "complete $LEAF/$EVENT"; else failed=$((failed+1)); say "FAILED $LEAF/$EVENT rc=$rc"; tail -30 "$LOG"; fi
done
