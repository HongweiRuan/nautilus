...
BATCH="${BATCH:-180}"
ROOT=/avl-west/navsafe-testsplit
TSV="${TSV:-scenes_testsplit.tsv}"                 # token \t log \t t0 \t t1 \t type \t source
TPL="templates/${STAGE}.yaml"
SEG=5000000                                  # 5.000s segments from t0 (drops the ~1.5s tail)

[ -f "$TSV" ] || { echo "!! $TSV missing — run mine_testsplit.py on a pod first"; exit 1; }
...
INFLIGHT=$(mktemp)
kubectl get pods -n "$NS" -l "app=navsafe-testsplit,stage=$STAGE" \
...
if [ "$n" -gt 0 ]; then
  ...
  for f in "$OUT"/*.yaml; do
    kubectl delete job -n "$NS" "navts-${STAGE}-$(basename "$f" .yaml)" --ignore-not-found --wait=false >/dev/null 2>&1
  done
  kubectl apply -f "$OUT"/
