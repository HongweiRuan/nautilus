#!/usr/bin/env bash
# Bulk-launch ONE pipeline stage for the 28 navhard scenario-type scenes at once,
# by plugging each scene's (token, log, t0, t1) into templates/<stage>.yaml and
# kubectl-apply. Each ~20s scenario is split into four ~5s segments (s1..s4)
# trained separately, so 28 scenarios => 112 jobs per stage. SPLIT=0 keeps whole scenes.
#
...
# Each navhard scenario is split into four EXACT 5s segments starting at t0
# (t0+k*5s; the ~1.5s tail of the ~21.5s window is dropped), converted/trained/
# rendered independently. Set SPLIT=0 to keep whole scenes. Segments get suffixed
# tokens s1/s2/s3/s4 so they are distinct scenes end to end (distinct ncore stores,
# usdz scene_ids, job names) -> 28 scenarios => 112 jobs.
emit () {  # <scene_id> <t0> <t1>
...
while IFS=$'\t' read -r C LOG T0 T1; do
  ...
  if [ "${SPLIT:-1}" != "0" ]; then
    SEG=5000000                             # exact 5s segments from t0 (drops the ~1.5s tail)
    P1=$(( T0 + SEG )); P2=$(( T0 + 2 * SEG )); P3=$(( T0 + 3 * SEG )); P4=$(( T0 + 4 * SEG ))
    if want "${C}s1"; then emit "${C}s1" "$T0" "$P1"; j=$((j+1)); fi
    if want "${C}s2"; then emit "${C}s2" "$P1" "$P2"; j=$((j+1)); fi
    if want "${C}s3"; then emit "${C}s3" "$P2" "$P3"; j=$((j+1)); fi
    if want "${C}s4"; then emit "${C}s4" "$P3" "$P4"; j=$((j+1)); fi
  else
...
NS="${NS:-cogrob}"
replace_one () { [ -n "${REPLACE:-}" ] && kubectl delete job -n "$NS" "nav5s-${STAGE}-$1" --ignore-not-found --wait=false >/dev/null 2>&1 || true; }
