...
# +/-0.5/1.0/1.5 m, longitudinal +/-0.5/1.0/1.5 m, yaw +/-2/5/10 deg. Every arm
...
#
#    --cache-size MUST BE AT LEAST 4, and that is a correctness floor, not a
#    speed knob. A NavSafe scenario is FOUR 5 s reconstructions the episode
#    renders in turn; with a smaller cache the renderer LRU-evicts one and
#    reloads it, and the reload does NOT carry the injected-actor registration
#    the eval made at scene load. Every scenario whose recipe inserts an actor
#    then dies on frame 0 with
#        INVALID_ARGUMENT: 'navsafe_animal#ph0' is not in list
#    scored as termination_reason=infra_failure. Measured on 2026-09-07: over
#    the same hours, all 13 of R-4's cells on the --cache-size 4 fleet scored,
#    and all 40 on the --cache-size 2 fleet failed. An earlier version of this
#    comment claimed the cache was only a speed trade-off. It is not.
#
#    That floor is why a one-card pod is no longer used for this campaign: the
#    renderer needs 18.5 GiB at --cache-size 4, which leaves too little of a
#    24 GB 3090 for IsaacSim and the policy beside it.
#
...
if [ "${N_GPU:-1}" -ge 2 ]; then
  RENDER_CACHE=4; EVAL_GPU=$((N_GPU - 1)); GPU_LAYOUT="$N_GPU cards, rollout on GPU $EVAL_GPU"
else
...
if grep -qE '^(recogdrive_il|recogdrive_rl|mtdrive_sft|mtdrive_mtgrpo|autovla|drivelaw|simwam|simwam_base)\b' "$MODELS_TSV"; then
