...
EVALUATOR=/root/BridgeSim/bridgesim/evaluation/unified_evaluator.py
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --normalization-json)      NORMALIZATION_JSON="$2";      shift 2 ;;
...
# ── Build Diffusion Planner fragment ─────────────────────────────────────────
DP_ARGS=()
[[ -n "${ARGS_JSON:-}"          ]] && DP_ARGS+=(--args-json          "$ARGS_JSON")
[[ -n "${NORMALIZATION_JSON:-}" ]] && DP_ARGS+=(--normalization-json "$NORMALIZATION_JSON")
...
[[ -n "${ARGS_JSON:-}"                 ]] && echo "  args-json        : $ARGS_JSON"
[[ -n "${NORMALIZATION_JSON:-}"        ]] && echo "  normalization-json: $NORMALIZATION_JSON"
