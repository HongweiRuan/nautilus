...
EXPECTED_TOKENS=${EXPECTED_TOKENS:-280}  # 28 for the proxy set (TOKENS_FILE=proxy28_tokens.txt)
[ "${#ALL_TOKENS[@]}" -eq "$EXPECTED_TOKENS" ] || { say "FATAL: expected exactly $EXPECTED_TOKENS canonical tokens, got ${#ALL_TOKENS[@]}"; exit 1; }
bundle_errors=0
...
[ "$bundle_errors" -eq 0 ] || { say "FATAL: $bundle_errors of ${#ALL_TOKENS[@]} bundles are incomplete"; exit 1; }
MY_TOKENS=()
...
# MODEL_FILTER: keep only the models.tsv rows whose label matches this ERE (whole label), e.g. to split a smoke across
# jobs by model. Unset = every row.
if [ -n "${MODEL_FILTER:-}" ]; then
  mapfile -t ROWS < <(printf '%s\n' "${ROWS[@]}" | awk -F'\t' -v re="^(${MODEL_FILTER})$" '$1 ~ re')
  [ ${#ROWS[@]} -gt 0 ] || { say "FATAL: MODEL_FILTER=$MODEL_FILTER matches no models.tsv row"; exit 1; }
fi
...
cp /cfg/drivor_action.py "$REPO/nexussim/policy/sensor/drivor_action.py"
sed -i '/"nexussim.policy.sensor.ltf",/a\    "nexussim.policy.sensor.drivor_action",' "$REPO/nexussim/engine/registry.py"
grep -q '"nexussim.policy.sensor.drivor_action"' "$REPO/nexussim/engine/registry.py" || { say "drivor_action not registered"; exit 1; }
...
# ── 1a. Robin's drivor_action agent (bosch-summer) ───────────────────────────
# His DrivoR fork + scripts/ (vocab_picker imports <repo>/scripts/drivor_action/train_vocab_picker.py, so the two keep
...
# Warm Omniverse Kit shader/material cache (pods/horuan-nexussim.yaml): without it the pod's first IsaacSim boot compiles
# materials and RtPso pipelines for ~8 min (measured 467 s cold vs 10.5 s restored). Keyed on driver version; a miss
# or a stale cache only means Kit recompiles, so nothing here aborts.
...
if [ -f "$KITCACHE" ]; then
  ...
  if [ -n "$KITPKG" ] && tar -xf "$KITCACHE" -C "$KITPKG" kit && tar -xf "$KITCACHE" -C / var root; then
    say "restored Kit shader/material cache $KITCACHE into $KITPKG"
  else
    say "Kit cache restore failed; the first IsaacSim boot will recompile (~8 min)"
  fi
else
  say "no Kit cache for this driver ($KITCACHE); the first IsaacSim boot will recompile (~8 min)"
fi
...
PYTHONPATH="$DA_ROOT/DrivoR:$PYTHONPATH" "$PY" -c "import torch, pytorch_lightning, navsim; assert navsim.__file__.startswith('$DA_ROOT/DrivoR'), navsim.__file__; from navsim.agents.drivor_action.drivor_action_agent import DrivorActionAgent; import sys; sys.path.insert(0, '$DA_ROOT/scripts/drivor_action'); import train_vocab_picker; print('drivor_action imports OK, torch', torch.__version__)" || { say "drivor_action import FAILED"; exit 1; }
      ...
      for _try in $(seq 1 "${CELL_TRIES:-4}"); do
        ...
        timeout --signal=KILL "$CELL_TIMEOUT" \
          "$PY" "$REPO/scripts/tools/eval_py123d.py" \
            ...
            --controller "${CONTROLLER:-lqr}" --execution-mode controller \
