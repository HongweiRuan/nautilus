...
def _overrides(deploy: Path, setting: str, refine: bool, dinov2_weights: str) -> tuple[list[str], list[str]]:
    ...
    if setting == "dinov2":
        ...
        raise ValueError(f"NAVSAFE_DA_SETTING must be dinov2 or best, got {setting!r}")
    ...
    ov = [
        ...
        "config.ext_scorer.kind=vocab_picker", f"config.ext_scorer.checkpoint_path={models[0]}",
...
class DrivorActionAdapter(SensorPolicy):
    ...
    def load_model(self) -> None:
        ...
        print(
            ...
            f"pickers={[Path(m).parent.name for m in models]}"
        )
