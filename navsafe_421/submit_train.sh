...
cd "$(dirname "$0")"
NS=cogrob; L="app=navhard421,stage=train"; BATCH=${BATCH:-100}   # submit up to this many per pass
...
done_l=$([ -n "$pod" ] && kubectl exec -n $NS "$pod" -- bash -c \
  'for f in /avl-west/navhard421/*/output_5cam/*/artifacts/last.usdz; do [ -f "$f" ] && echo "$f"; done' 2>/dev/null \
  | sed -E 's#.*/navhard421/([^/]+)/output_5cam.*#\1#')
succ_l=$(kubectl get jobs -n $NS -l "$L" -o jsonpath='{range .items[?(@.status.succeeded==1)]}{.metadata.name}{"\n"}{end}' 2>/dev/null | sed 's/navhard-train-//')
...
inflight_l=$(kubectl get pods -n $NS -l "$L" -o jsonpath='{range .items[*]}{.status.phase}{"|"}{.metadata.labels.job-name}{"\n"}{end}' 2>/dev/null \
  | awk -F'|' '$1=="Running"||$1=="Pending"||$1=="ContainerCreating"{print $2}' | sed 's/navhard-train-//')
...
while IFS= read -r C; do
  ...
  kubectl delete job -n $NS "navhard-train-$C" --ignore-not-found --wait=false >/dev/null 2>&1
